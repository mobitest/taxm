  	...
  	dbSize: 5 * 1024 * 1024, // 5MB
		...
		createTable:function(){
			this.db.transaction(function(tx) {
		  	...
		  	var table_columns = "(ID INTEGER PRIMARY KEY ASC, name text unique, content TEXT, update_time DATETIME, fav int default 0)";
		    tx.executeSql("CREATE TABLE IF NOT EXISTS corp_jbxx" +table_columns, [],dbutil.onSuccess);
		    tx.executeSql("CREATE TABLE IF NOT EXISTS corp_hd"+table_columns, [],dbutil.onSuccess);
		    tx.executeSql("CREATE TABLE IF NOT EXISTS corp_ns"+table_columns, [],dbutil.onSuccess);
		    tx.executeSql("CREATE TABLE IF NOT EXISTS corp_sb"+table_columns, [],dbutil.onSuccess);
		    tx.executeSql("CREATE TABLE IF NOT EXISTS corp_fpgz"+table_columns, [],dbutil.onSuccess);
		    tx.executeSql("CREATE TABLE IF NOT EXISTS corp_sbfjf"+table_columns, [],dbutil.onSuccess);
		  });
		},
		/*写行*/
		...
		/*收藏：name企业名称，fav收藏标志，0=不收藏，1=收藏*/
		favRow: function(name, fav, callback){
			var succ = callback? callback: dbutil.onSuccess;
			this.db.transaction(function (tx){
				tx.executeSql('update corp_jbxx set fav=? where name=?', [ fav, name],
					callback,
		      dbutil.onError);
			});
		...
		cleanCache:function(){
	    	...
	    	tx.executeSql('delete from corp_ns where name not in(' + base_rows_sql + ') ',[],	dbutil.onSuccess,dbutil.onError);
				console.log("clear corp_sb");
	    	tx.executeSql('delete from corp_sb where name not in(' + base_rows_sql + ') ',[],	dbutil.onSuccess,dbutil.onError);
				console.log("clear corp_fpgz");
	    	tx.executeSql('delete from corp_fpgz where name not in(' + base_rows_sql + ') ',[],	dbutil.onSuccess,dbutil.onError);
				console.log("clear corp_sbfjf");
	    	tx.executeSql('delete from corp_sbfjf where name not in(' + base_rows_sql + ') ',[],	dbutil.onSuccess,dbutil.onError);
   		});
		},
		...
		/*用名称关键字查询*/
		findByName: function( tablename, keyword, callback, exactly) {
			this.db.transaction(function (tx){
				var sql  = "select id,name,content,update_time,fav from " + tablename + " where name like '%"+ keyword + "%' order by fav desc, update_time desc";
				if(exactly) sql =  "select id,name,content,update_time,fav from " + tablename + " where name = '"+ keyword + "' order by fav desc, update_time desc";
			...
			//3生成页面 render
				...
				load:function(info_type, id, remoteNow){
					...
					dbutil.findByName("corp_"+ info_type, id, function(tx, r){
						...
						if(r.rows.length ==0 || r.rows.item(0).content.length ==0){
							console.log("cache not found..." + info_type)
				...
				*/
				getRemote:function(info_type, id, path){
					$.mobile.loadingMessage =   " 远程数据获取中（"  + info_type+ "）" ; 
					$.mobile.showPageLoadingMsg();
					
					$.ajax({url: path,					
							dataType:DEBUG_LOCAL? "json":"jsonp",timeout:TIMEOUT,
